    ...
    <div className="editor-picture__pan max-w-[100vw] w-[360px]">
      <SignInSignUpContainer ref={container}>
        ...
        <SimpleCard className="text-center flex flex-col gap-dashboard-spacing-element-medium w-[900px] max-w-[100vw]">
          <div className={`flex items-stretch gap-dashboard-spacing-element-medium transition-transform ${isMobile && mobileStep === 1 ? '-translate-x-[calc(100%+32px)]' : ''}`}>

            <div 
              className={`${isMobile ? 'basis-full shrink-0' : 'basis-1/2 shrink-1'}`}
            >
              <div 
              ...
              >
                ...
                <GeneratedAvatar label={context.initials} className="pointer-events-auto" img={imageToDisplay}/>
                <MentionInteraction className='pointer-events-auto' >Ajoutez votre photo de profil</MentionInteraction>
              </div>
            </div>

            {!isMobile && <div className="border-l border-l-dashboard-separator-white-low-opacity w-[0px]"></div>}

            <div className={`${isMobile ? 'basis-full shrink-0' : 'basis-1/2 shrink-1'}`}>
              <Input
                type="textarea"
                bg="light"
        ...
        <div className="flex flex-col items-center w-full">
          <Button
            ...
            disabled={!isMobile ? !(context.editorPictureOk && context.editorDescriptionOk) : false}
            className="w-full"
          />
