    ...
    <div className="persoInfo__pan max-w-[100vw] w-[424px]">
      <SignInSignUpContainer ref={container}>
        <div className="w-full flex flex-col items-center gap-dashboard-spacing-element-medium px-dashboard-specific-radius md:p-0">
          ...
          <div className='text-large text-center'>Commencez avec editYour.Film </div>

          <div className="flex items-center flex-col sm:flex-row gap-dashboard-button-separation-spacing">
            {context.accountType && ['creator', 'both'].includes(context.accountType) && <InfoMessage message="Profil Créateur.rice" Icon={VideoIcon} small/>}
            {context.accountType === 'both' && <span className="hidden sm:block text-dashboard-text-description-base">&</span>}
            {context.accountType && ['editor', 'both'].includes(context.accountType) && <InfoMessage message="Profil Monteur.se" Icon={SlidersIcon} small/>}
          </div>
          <hr className='form-separator'/>
        </div>

        <SimpleCard paddingMobileSmall className="text-center flex flex-col gap-dashboard-spacing-element-medium">
          <div>
            <div className="text-dashboard-text-title-white-high text-medium font-medium">Complétez vos informations personnelles</div>
            <div className="text-dashboard-text-description-base mt-dashboard-mention-padding-right-left">Seul votre nom d'utilisateur est public. Tous les monteurs inscrits sont répertoriés dans le catalogue avec leur nom d'utilisateur.</div>
          </div>
          ...
          <div className="flex flex-col items-stretch gap-dashboard-button-separation-spacing">
            ...
            <Input 
              ...
              noLabel
              className={`font-medium text-base ${context.userNameAvailable ? 'opacity-100' : 'opacity-50'}`}
              textSunset
            ...
            <InfoMessage wFull message={context.userNameMessage?.message} Icon={context.userNameMessage?.Icon} />
        ...
        <div className="w-full px-dashboard-specific-radius md:p-0">
          <Button
            ...
            disabled={!(context.f_name && context.l_name && context.userNameAvailable)}
            className="w-full"
          />
        </div>
