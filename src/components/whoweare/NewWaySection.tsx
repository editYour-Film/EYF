...
export const NewWaySection = ({ data }: any) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const imgTl1: GSAPTimeline | undefined =
        scrollAnim && img1.current && img1W.current
          ? setImgTl(img1.current, img1W.current, 1)
          : undefined;
      const imgTl2: GSAPTimeline | undefined =
        scrollAnim && img2.current && img2W.current
  ...
  return (
    <div
      ref={section}
      className="two-imgs relative grid grid-cols-[1fr] grid-rows-[auto] lg:grid-cols-[1fr_1fr] lg:grid-rows-[1fr_0.5fr_0.5fr_1fr] gap-x-44 my-10 md:my-40 z-10 w-full overflow-hidden"
    >
      <div
        className={`px-4 lg:row-start-1 lg:row-end-2 lg:col-start-1 lg:col-end-2 lg:justify-self-center xl:max-w-md ${
          inView && "inView"
        }`}
      >
        <H2 arrow fake>
