...
export const MoreArticles = ({ articles, current }: any) => {
  ...
  useEffect(() => {
    ctx.current && ctx.current.add('goTo', (i: number) => {   
    ...
    ctx.current && ctx.current.add('dragTo', () => {
      gsap.set(wrapper.current, {
        x: initOffset - offset * currentSlide + dragOffset.current,
      })
    })

    ctx.current && ctx.current.add('reset', () => {
      if(tl.current) {
        tl.current.kill()
      }
      gsap.set(wrapper.current, {
  ...
  const getValues = () => {
    const gapValue = wrapper.current ? parseInt(getComputedStyle(wrapper.current).gap, 10) : 0
    const elW = cardArticles.current[0].current ? cardArticles.current[0].current?.offsetWidth : 0
  ...
  const next = () => {
    if (!isTweening) currentSlide < availableArticles.length - 1 && setCurrentSlide(currentSlide + 1)
  }
