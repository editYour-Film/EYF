import { RefObject, SyntheticEvent, useContext, useEffect, useRef, useState } from "react"
...
export const ModifyVideoPanel = () => {
  ...
  <div
    ref={titleEl}
    onClick={() => { !descriptionEnabled && setTitleEnabled(true) }}
    className="w-full"
  >
    <div className={`${titleEnabled ? 'hidden' : ''} text-dashboard-text-description-base hover:text-dashboard-text-title-white-high cursor-pointer`}>
  ...
  <div
    ref={contentEl}
    onClick={() => { !descriptionEnabled && setDescriptionEnabled(true) }}
    className="relative"
  >
    <div className={`${descriptionEnabled ? 'opacity-0 pointer-events-none' : ''} text-dashboard-text-description-base hover:text-dashboard-text-title-white-high cursor-pointer`}>
      {editorContext.modelDescription}
    </div>
    <div className={`absolute top-0 h-full w-full ${descriptionEnabled ? '' : 'hidden'}`}>
      <Input 
        type='textarea'
        bg="black"
  ...
  useEffect(() => {
    const handleClickOutsideRef = (e:any, ref:RefObject<HTMLDivElement>, setter:Function) => {
  ...
  return (
    <div 
      className="modify-video fixed flex justify-center top-0 left-0 w-full h-full z-popup overflow-hidden pointer-events-none"
    >
      <OverlayModel 
        className="min-h-screen h-max"
        toggle={editorContext.showModifyPanel}
        onClose ={() => { editorContext.hideModifyPanel() }}
        onClosed={() => { editorContext.setCurrentModelToModify(undefined) }}
      >
        <div className='modify-video__inner min-h-full p-dashboard-button-separation-spacing md:p-0 bg-dashboard-button-dark md:border-03 md:rounded-dashboard-button-separation-spacing grid grid-modify-video pb-[50px] overflow-hidden'>
            ...
            <>
              {isMobile && 
                <IslandButton
                  type="tertiary"
                  Icon={Close}
                  onClick={() => { editorContext.hideModifyPanel() }}
                  iconColor="appleRed"
              ...
              >
                <IslandButton 
                  ...
                  Icon={Trash}
                  onClick={() => {
                    editorContext.currentModelToModify && editorContext.handleDeleteVideo(editorContext.currentModelToModify.id)
                  }}
                />
                ...
                <IslandButton 
                  ...
                  label="Retirer"
                  onClick={() => {
                    editorContext.currentModelToModify && editorContext.handleDisableVideo(editorContext.currentModelToModify.id)
                  }}
                />
                ...
                <IslandButton 
                  ...
                  onClick={() => {
                    editorContext.currentModelToModify && editorContext.storeHighlightedVideo(editorContext.currentModelToModify.id)
                  }}
                />
                  ...
                  <div className="relative">
                    <ToolboxButton
                      ...
                      label={
                        ...
                        ? editorContext.modelFormat.length > 20 ? editorContext.modelFormat.slice(0, 16) + '...' : editorContext.modelFormat
                    ...
                    />
                    {
                      ...
                      <DropBox
                        ...
                        setToggle={ (val) => { setOpenFrameToolbox(val) }}
                  ...
                  <div className="relative">
                    <ToolboxButton
                      ...
                      label={
                        editorContext.modelAudio
                          ? editorContext.modelAudio.length > 20 ? editorContext.modelAudio.slice(0, 16) + '...' : editorContext.modelAudio
                          : 'Ajoutez le format de la vidéo'
                        }
                      onClick={() => { setOpenLangToolbox(!openLangToolbox) }}
                    />
                    ...
                    {!isMobile &&
                      <DropBox
                        ...
                        setToggle={ (val) => { setOpenLangToolbox(val) }}
                        className="hidden md:block absolute top-0 left-1/2"
                      />
                    }
                  ...
                  <div className="relative">
                    <ToolboxButton
                      ...
                      label=
                        ...
                        editorContext.modelWorkTime
                          ? editorContext.modelWorkTime.length > 20 ? editorContext.modelWorkTime.slice(0, 16) + '...' : editorContext.modelWorkTime
                          : 'Ajoutez le format de la vidéo'
                        }
                          
                      onClick={() => { setOpenWorktimeToolbox(!openWorktimeToolbox) }}
                    />
                    ...
                    {!isMobile &&
                      <DropBox
                        ...
                        setToggle={ (val) => { setOpenWorktimeToolbox(val) }}
                        className="hidden md:block absolute top-0 left-1/2"
                      />                   
                    }
                  ...
                  <div className="relative">
                    <ToolboxButton
                      ...
                      label={
                        editorContext.modelSoftware
                          ? editorContext.modelSoftware.join(', ').length > 20 ? editorContext.modelSoftware.join(', ').slice(0, 16) + '...' : editorContext.modelSoftware.join(', ')
                          : 'Ajoutez un logiciel'
                      }
                      onClick={() => { setOpenSoftwareToolbox(!openSoftwareToolbox) }}
                    />
                    ...
                    {!isMobile &&
                      <DropBox
                        ...
                        setToggle={ (val) => { setOpenSoftwareToolbox(val) }}
                        className="hidden md:block absolute top-0 left-1/2"
                      />                
                    }
                      ...
                      <span className="text-blueBerry">
                        {
                          ...
                          ? editorContext.outLink.length > 20 ? editorContext.outLink.slice(0, 16) + '...' : editorContext.outLink
                          : 'Ajoutez un Lien'
                        }
                      </span>}
                      onClick={() => { setOpenLinkToolbox(!openLinkToolbox)}}
                    />
                    {!isMobile &&
                      <DropBox
                        type="link"
                        ...
                        setToggle={ (val) => { setOpenLinkToolbox(val) }}
                ...
                <div>
                  <div className="modify-video__tags relative w-full flex flex-row md:flex-col gap-dashboard-mention-padding-right-left pr-dashboard-mention-padding-right-left py-dashboard-mention-padding-right-left overflow-scroll md:overflow-visible">
                    ...
                    <div className="flex shrink-0 gap-[10px] pr-[20px] md:w-full items-center justify-between">
                      <div className="text-dashboard-text-description-base-low">Mots-clés</div>
                    ...
                    {!isMobile &&
                      <DropBox
                        ...
                        setToggle={ (val) => { setOpenTagToolbox(val) }}
                        className="hidden md:block md:absolute md:top-0 md:left-1/2"
                      />
                    }

                    {editorContext.tags && editorContext.tags.map((tag) => {                      
                      return (
                        <Keyword
                          text={tag.name}
                          icon="cross"
              ...
              <div className="modify-video__headline md:col-[content-start_/_content-end] row-[2_/_3] md:row-[3_/_4] align-self-auto md:py-dashboard-spacing-element-medium  md:pl-dashboard-spacing-element-medium flex flex-col gap-dashboard-spacing-element-medium">
                <div className="flex gap-dashboard-button-separation-spacing">
                  ...
                  <div className="flex flex-col">
                    <div className="w-full text-title-medium n27 uppercase leading-none"> {titleContent} </div>
                    <div className="text-dashboard-text-description-base-low text-base">Par {editorContext.currentModelToModify.user_info.f_name}{editorContext.currentModelToModify.user_info.l_name} </div>
                  </div>
          ...
          <div className="row-[7_/_8] md:row-[4_/_5] md:col-[content-start_/_content-end] flex flex-col gap-[39px] pb-[39px]">
            <IslandButton 
              ...
              type="primary"
              label="Enregistrer"
              onClick={() => { editorContext.updateCurrentModel() }}
...

            {
              isMobile &&
                <IslandButton 
                  ...
                  onClick={() => { editorContext.currentModelToModify && editorContext.handleDeleteVideo(editorContext.currentModelToModify.id) }}
                  className="relative w-max ml-auto mr-0 place-self-end"
                />
            }
          </div>
...
const ToolboxButton = ({Icon, label, onClick}:ToolboxButtonProps) => {
  return (
    ...
    >
      <div><Icon className='w-[24px] h-[24px] flex'/></div>
