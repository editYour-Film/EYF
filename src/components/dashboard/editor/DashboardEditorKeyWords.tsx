    ...
    <div className="dashboard-editor-keywords flex flex-col gap-dashboard-spacing-element-medium px-dashboard-mention-padding-right-left md:px-0 w-full">
      ...
      { 
        ...
        ?
          <div>
            <InfoMessage 
              message={<><p>Vous n'êtes actuellement par visible dans le catalogue.</p><p>Ajoutez un modèle et choisissez des mots-clé pour être affiché dans les filtres du catalogue. Vous pouvez gérer votre visibilité depuis cette section.</p></>}
              bg='black'
              wFull
              Icon={Eye}
            />
        ...
        :
          <div className="db-editor__tags flex flex-wrap w-full gap-3">
            {context.keywords && context.keywords.map((keyword, i) => {
              return (
                <Keyword 
                  key={i}
                  text={keyword.name}
