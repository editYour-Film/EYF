  ...
  return (
    <div>
      ...
      <div className="addModel__content relative rounded-3xl mt-12 p-5 border overflow-hidden">
        <div className={`addModel__bg absolute top-0 left-0 w-full h-full z-0 ${[0,3].includes(context.currentStep) ? 'bg-alphaWhite bg-opacity-10' : 'bg-black'}`}></div>
    ...
    <div className="steps flex justify-between">
      ...
      <div className="steps__steps relative">
        <div className="flex relative justify-between gap-10 z-10">
          ...
          {context.currentStep === 3 && <div className="px-4 flex bg-black rounded-full justify-center items-center border">Modèle enregistré</div>}
        </div>
...
const Step = ({index, title}:{index:number, title:string}) => {
  ...
  return (
    <div className={`step flex justify-between items-center ${context.currentStep === 3 ? '' : 'gap-[10px]'} pr-[15px] w-max bg-black rounded-full border`}>
      <div className={`step-number rounded-full n27 w-7 h-7 flex justify-center items-center ${context.currentStep === 3 ? 'bg-[#858585]' : 'gap-[10px]'} ${isActive ? 'bg-violet text-white' : 'bg-white text-black'}`}>{index + 1}</div>
