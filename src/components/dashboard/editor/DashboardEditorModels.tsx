...
export const DashboardEditorModels = ({models}:DashboardEditorModelsProps) => {  
  ...
  return (
    <div className="dashboard-editor-models">
      <div className="dashboard-editor-models__head flex flex-col sm:flex-row flex-wrap sm:justify-between sm:items-center">
        ...
        <Button 
          variant='primary'
          text={models && models.length ? 'Ajouter un modèle' : 'Pourquoi ajouter un modèle ?'}
          onClick={
            () => { 
      ...
      <div className={`dashboard-editor-models__models ${models && models.length && 'grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-3 gap-y-4 lg:gap-y-12'} mt-12`}>
        {models && models.length ? 
          models.map((model:any, i:number) => {
          return <VideoModel 
            key={i} 
            data={model.attributes.video.data} 
            thumbnail={model.attributes.thumbnail.data ? model.attributes.thumbnail.data.attributes.url : context.defaultImage} 
            onModify={() => { handleModify(model.id)}}
            onDelete={() => { handleDelete(model.id)}}
          />
          }) :
          <div className={`dashboard-editor-models__placeholder relative flex flex-col-reverse md:flex-row lg:flex-col-reverse xl:flex-row md:items-center lg:items-stretch gap-8 gradient-pastel w-full rounded-3xl p-4 lg:p-8 xl:p-14`}>
            ...
            <div className="dashboard-editor-models__content text-black basis-5/12 flex flex-col gap-4">
              <div className="n27 text-2xl font-medium ">Devenez visible, ajoutez un modèle</div>
