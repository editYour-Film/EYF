...
export const FileInput = ({}:FileInputProps) => {
  ...
  const handleSubmit = async (e:FormEvent<HTMLFormElement>) => {
    ...
    const resEditorVideo = await useStrapiPost('editor-videos', formData, user[1], true)    
    setLoading(false)
        ...
        >
          <div className="flex flex-col gap-8 px-14 py-16 items-center">
            ...
            <div className="text-center flex flex-col items-center gap-4">
              <div className="n27 text-2xl">Glissez-déposez le fichier que vous souhaitez mettre en ligne</div>
              <div className="text-base-text w-7/12">Vos vidéos resteront privées jusqu’à leur publication. Chaque modèle est vérifié avant d’être publié dans le catalogue.</div>
            ...
            <form 
              ref={form}
              className="absolute top-0 left-0 w-full h-full z-0" action=""
              onSubmit={(e) => { handleSubmit(e) }}
            > 
            <label htmlFor="video" className="opacity-0 "> Sélectionnez une video </label>
            <input
              ref={input}
              type="file" 
              name="video" 
