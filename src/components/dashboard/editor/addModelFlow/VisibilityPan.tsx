...
export const VisibilityPan = () => {
  ...
  const visibilityOptions = [{
    value: 'public',
    label: 'Publique',
    helper: 'Tout le monde peut voir votre vidéo affichée dans le catalogue lorsque vous avez renseigné vos disponibilités depuis votre agenda.'
  },{
    value: 'unrepertoried',
    label: 'Non répertoriée',
    helper: 'Tous les utilisateurs disposant du lien peuvent regarder votre vidéo'
  },{
    value: 'private',
    label: 'Privée',
    helper: 'Seul vous pouvez regarder votre vidéo depuis votre espace monteur.'
  }]

  const durationOptions = [{
    value: 'base',
    label: 'Basique',
    helper: 'Recherche minimale et peu de temps nécessaire, montage simple et rapide.'
  },{
    value: 'medium',
    label: 'Moyen',
    helper: 'Recherche modérée, temps raisonnable, montage nécessitant une certaine compétence.'
  },{
    value: 'high',
    label: 'Très compliqué',
    helper: 'Recherche intensive, temps substantiel, montage complexe nécessitant des compétences avancées.'
  }]

  const [visibilityValue, setVisibilityValue] = useState<string|undefined>(undefined)
  ...
  useEffect(() => {
    if(context.strapiObject) {
      setVisibilityValue(context.strapiObject.attributes.visibility ?? visibilityOptions[0].value)
      setCopyWriteValue(context.strapiObject.attributes.copywrite ?? '')
      setDurationValue(context.strapiObject.attributes.duration ?? durationOptions[0].value)
    }
  ...
  return (
    <div className="infos-pan bg-black p-5 flex flex-col gap-8">
      <div className="info-pan__video-w relative h-0 pb-[57.6%] rounded-2xl overflow-hidden border">
        ...
        >
          <source src={context.strapiObject?.attributes.video.data.attributes.url} type={context.strapiObject?.attributes.video.data.attributes.mime}/>
        </video>
      </div>
      <div className="info-pan__video-info">
        <div className="flex justify-between items-baseline flex-wrap">
          <div className="display flex items-baseline gap-4 n27">
            <div className="text-lg font-medium uppercase tracking-widest">{context.modifiedData?.title}</div>
            <div className="text-sm">&#40;{videoDuration?.min !== 0 && videoDuration?.min.toString() + 'minutes'}{videoDuration?.sec.toString()}{videoDuration?.min === 0 && 'secondes'}&#41;</div>
          </div>
          <div className="text-base-text font-light text-[17px] uppercase n27">{user[0].details.f_name}</div>
        </div>
        <div className="mt-3 text-sm">Lien de la video <Link className="text-primary-high text-opacity-80 hover:text-opacity-100" href="https://edityour.film/">https://edityour.film/lienDeLaVideo</Link></div>
      </div>
      <hr />
      <form 
        ref={form}
        className="info-pan__format flex flex-col gap-8"
      >
        <Input
          label='Visibilité' 
          type="radioColumn"
      ...
      <div>
        <Input
          ...
          className="h-[170px]"
        />
        { copywriteError && <div className="text-error text-sm mt-2">{copywriteError}</div>}
      </div>
      ...
      <div className="flex justify-center sm:justify-between items-center flex-wrap gap-8">
        <div>
          {error ? 
            <span className="text-error text-sm ">Le formulaire contient des erreurs</span> : 
            <span className="text-base-text text-sm ">Vérifications terminées. Aucun problème détecté.</span>}
        </div>
        <Button 
          variant="black"
          text="Suivant"
          icon="arrow-right"
          iconRight
          className={`w-max ${error ? 'pointer-events-none opacity-10': 'bg-black opacity-50'}`}
          onClick={() => { 
            !error && handleSubmit() 
          }}
        />
