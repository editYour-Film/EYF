...
export const InfosPan = ({}:InfosPanProps) => {
  ...
  useEffect(() => {
    if(context.strapiObject) {
      ...
      setIsHighlightedValue((user[0].details.highlighted_video.data && user[0].details.highlighted_video.data.id === context.strapiObject.id) ? highlightedOptions[0].value : highlightedOptions[1].value)
  ...
  return (
    <div className="infos-pan bg-black p-5 flex flex-col gap-8">
      ...
      <div className="info-pan__video-w relative h-0 pb-[32%] rounded-2xl overflow-hidden border">
        ...
        >
          {entry && <source src={entry.video.data.attributes.url} type={entry.video.data.attributes.mime}/>}
        </video>
      </div>

      <div className="info-pan__title flex items-baseline gap-2">
        <div className="n27 text-lg font-medium">{entry?.title}</div>
        <div className="n27 text-sm font-light">&#40;{duration?.min !== 0 && duration?.min.toString() + 'minutes'}{duration?.sec.toString()}{duration?.min === 0 && 'secondes'}&#41;</div>
      </div>
          ...
          <div className="md:basis-7/12 flex flex-col justify-stretch gap-8">
            <div>
              <Input
                ...
                maxlength={100}
              />
              { titleError && <div className="text-error text-sm mt-2">{titleError}</div>}
            </div>
            ...
            <div>
              <Input
                ...
                maxlength={100}
              />
              { descriptionError && <div className="text-error text-sm mt-2">{descriptionError}</div>}
            </div>
          </div>
      ...
      <div className="infos-pan__tag-container flex flex-wrap gap-2">
        { tags && tags.length > 0 && tags.map((tag:any, i:number) => {
          return (
            <Tag
              key={i}
              icon
...
const KeyWords = ({onChange}: keyWordsProps) => {
  ...
  const { data: strapiTags, mutate: getStrapiTags } = useStrapi("video-tags", false);
