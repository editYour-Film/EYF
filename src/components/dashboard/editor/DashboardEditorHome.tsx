...
const AddModelContextProvider: React.FC<any> = (props) => {
  const user = useUser();
...
export const DashboardEditorHome = ({}: DashboardEditorHomeProps) => {
  return (
...
const Content = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      // eslint-disable-next-line react-hooks/rules-of-hooks
      const promise = await useStrapiGet(
        `user-infos/${user[0].details.id}?populate[highlighted_video][populate]=*`,
        user[0]
      );
      setHighLightedVideo(
        promise.data.data.attributes.highlighted_video.data
          ? promise.data.data.attributes.highlighted_video.data.attributes
          : false
      );
    };
