...
import { UserNameInput, FNameInput, LNameInput, DescInput, PhoneInput, EmailInput, StreetInput, ZipCodeInput, CityInput } from "./profil/inputs"
...
export const Inputs = () => {
  ...
  const [dayOptions, setDayOptions] = useState<ReactElement[] | undefined>(undefined)

  const [month, setMonth] = useState<string | undefined>(undefined)
  const [monthOptions, setMonthOptions] = useState<ReactElement[] | undefined>(undefined)
  
  const [year, setYear] = useState<string | undefined>(undefined)
  const [yearOptions, setYearOptions] = useState<ReactElement[] | undefined>(undefined)

  useEffect(() => {    
    month && setDayOptions(generateDayOptions(months.indexOf(month)))
  }, [month])
    ...
    for (let i = 1; i <= days[monthIndex]; i++) {
      options.push(
        <option 
          ...
          value={i.toString()} selected={day === i.toString()}>
            {i}
          </option>
      )
    }
  ...
  const generateMonthOptions = () => {
    ...
    for (let i = 0; i < months.length; i++) {
      options.push(
        <option 
          className='text-black'
          value={months[i]} selected={month ? month === months[i] : i === 0}>
            {months[i]}
          </option>
      )
    }    
    ...
    for (let i = 0; i <120; i++) {
      options.push(
        <option 
          className='text-black'
          value={actualYear - i} 
          selected={year ? parseInt(year) === actualYear - i : i === 0}>
            {actualYear - i}
        </option>
      )
    }    
  ...
  <>
    ...
    <div className="flex flex-col gap-dashboard-button-separation-spacing">
      <div className="text-small text-dashboard-text-description-base">Date de naissance</div>
