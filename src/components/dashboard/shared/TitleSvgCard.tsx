...
export const TitleSvgCard = ({type, title, text, img, className, hideImgMobile, Svg}: TitleSvgCardProps) => {
  return (
    <div className={`dashboard-editor-cover__placeholder relative flex flex-col items-center xl:flex-row justify-center ${type === 'imgBottomRight' ? 'xl:justify-start' : ''} gap-[30px] border bg-black rounded-dashboard-button-square-radius lg:mb-0 overflow-hidden ${className} ${type !== 'imgBottomRight' ? 'px-[53px] py-[61px]' : ''}`}>
      ...
      <div 
        aria-hidden
        className={`shrink-0 ${type === 'imgBottomRight' ? 'xl:absolute xl:right-0 bottom-0 h-full w-full xl:max-w-[480px]' : 'md:self-stretch w-full xl:w-4/12 2xl:w-5/12 xl:max-w-[500px] fullHd:max-w-[300px] '}`}
      >
        <div className={`${hideImgMobile ? 'hidden md:block' : ''} ${type === 'imgBottomRight' ? 'relative right-0 bottom-0 h-full' : 'relative w-full h-auto min-h-[150px] basis-[40%]'} shrink-0 z-0`}>
          {Svg ?
            <Svg 
              alt={'Illustration'}
            />
