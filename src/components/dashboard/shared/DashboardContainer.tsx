...
import { DashBoardContext, dashBoardPanelType } from "../_context/DashBoardContext"

type DashboardContainerProps = {
  className?: string
}

export const DashboardContainer = ({children, className}:PropsWithChildren<DashboardContainerProps>) => {
  const context = useContext(DashBoardContext)

  return (
    <div className={`dashboard_container flex flex-col gap-dashboard-spacing-element-medium md:px-padding-medium pb-padding-medium bg-dashboard-background-content-area md:border-03 rounded-dashboard-button-square-radius shadow-large ${className ?? ''}`}>
