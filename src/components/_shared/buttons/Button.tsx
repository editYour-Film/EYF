...
export const Button:React.FC<ButtonProps> = ({type, label, Icon, disabled = false, className, onClick}: ButtonProps) => {  
  ...
  return (
    <button 
      className={`button ${baseStyle} ${typeStyle} ${focusedStyle} ${disabled ? disabledStyle : ''} ${className ?? ''}`}
      onClick={() => { onClick(); }}
      disabled={disabled}
    >
      {Icon && 
