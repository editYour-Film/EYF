...
export const Title = forwardRef<any, TitleAnimProps>(function Title(
  { text, type, className, isAnim, isSunset, split, onSplitted },
  ref
) {
  ...
  return (
    <RenderSwitch condition={type} className={className ?? ""}>
      {split ? (
        <TextSplit
          ref={title}
          input={text}
...
const RenderSwitch = forwardRef<
  HTMLHeadingElement | HTMLParagraphElement | HTMLSpanElement,
  PropsWithChildren<RenderSwitchProps>
>(function RenderSwitch({ condition, children, className }, ref) {
  switch (condition) {
    case "h1":
      return (
        <h1
