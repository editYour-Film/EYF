import { skillsInterface } from "@/components/dashboard/_context/ProfilContext";
...
type SelectProps = {
  ...
  label: string,
  helperTop?: string,
  list: skillsInterface[] | undefined,
  onAdd?: (val: string) => void,
  onSelectOption: (val: string) => void,
};

export const Select = ({search = false, placeholder, label, helperTop, list, onAdd, onSelectOption}: SelectProps) => {
    ...
    const handleSearch = (q: string) => {
      list && setVisibleItems(list.filter((i) => i.label.toLowerCase().includes(q.toLowerCase() as string)))
    }
    ...
    return (
      <div 
        className="group relative focus-within:z-20"
      >
        <label 
          htmlFor={id}
          className="block text-dashboard-text-description-base text-small mb-dashboard-button-separation-spacing"
        >{label}</label>
        {helperTop && <p className={helperClass + ' mb-dashboard-button-separation-spacing'}>{helperTop}</p>}
        ...
        <div 
        lenis-prevent
        className={`absolute top-full w-full bg-dashboard-background-content-area group-focus-within:border rounded-b-dashboard-button-square-radius h-0 overflow-hidden  ${visibleItems && visibleItems.length > 0 ? 'group-focus-within:h-auto group-focus-within:p-dashboard-button-separation-spacing max-h-[130px] group-focus-within:overflow-scroll' : ''} no-scroll-bar`}>
