...
export const Dropdown = ({label, title, onChange, options, selected = [], mandatory, open, className = ''}: DropdownProps) => {  
  ...
  const handleOpenDropdown = (isOpen:boolean, immediate = false) => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(dropdownOptions.current, {
      ...
      tl.fromTo(dropdownOptionsInner.current, {
        yPercent: isOpen ? 100 : 0,
      },{
        yPercent: isOpen ? 0 : 100,
        duration: !immediate ? 0.5 : 0,
  ...
  const handleKeyDown = (e:any) => {    
    if(e.key === 'Enter') {
      e.preventDefault()
      if(isOpen) selectedOption && handleAddOption(options[selectedOption])
      else setIsOpen(true)
    } 
      ...
      if(currentIndex < options.length - 1) {
        ...
        optionslist.current?.scrollTo({top: optionsRef.current[currentIndex + 1].current.offsetTop - 40})
      }
    }
  ...
  const handleAddOption = (option: optionInterface) => {    
    !selected?.includes(option) && onChange([...selected, option])
  }
  ...
  return (
    <div
      ...
      className={`dropdown relative ${isOpen ? 'z-50' : ''} ${className} w-full`}
      onKeyDown={(e) => {
        handleKeyDown(e)
      }}
    >
      ...
      <div className="text-medium mb-dashboard-button-separation-spacing">{title}</div>
      ...
      <div 
        ref={dropdownOptions}
        className={`dropdown__options-w w-[calc(100%+2px)] overflow-hidden top-5 z-10 box-border ${!isOpen && 'pointer-events-none'} mt-dashboard-button-separation-spacing`}
      >
        <div 
          ref={dropdownOptionsInner}
          className={`dropdown__options-inner w-full bg-black rounded-b-xl ${enableSearch && 'border-white'}`}
        >
            ...
            {currentOptions.map((option, i:number) => {
              return (
                <Tag
                  ...
                  icon={option.icon}
                  text={option.label}
                  className={`dropdown_tag hover:bg-alphaWhite-300 ${selectedOption === i ? 'bg-alphaWhite-300' : ''} w-full ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}
                  wFull
                  size="sm"
                />
              )
