...
import {
  EditorVideo,
  modelType,
} from "@/components/dashboard/editor/_context/EditorContext";
    ...
    <>
      {video ? (
        <div className="model-large bg-dashboard-button-dark rounded-dashboard-button-separation-spacing overflow-hidden border border-transparent">
          ...
          >
            {video.thumbnail && video.thumbnail.data && (
              <Image
          ...
          <div className="model-large__content relative p-padding-medium border-t z-10">
            <div className="model-large__infos w-full flex flex-row justify-between">
              {type !== "default" ? (
                <>
                  <div className="text-dashboard-text-title-white-high text-base font-medium">
                    {video.title}
                  </div>
                ...
                <div className="">
                  ...
                  <div>
                    <div>{video.title}</div>
                    <div>
                      par {(video.user_info as any).data.attributes.f_name}{" "}
                      {(video.user_info as any).data.attributes.l_name}
                    </div>
                  </div>
                </div>
              )}
