...
export const Cursor = () => {
  ...
  const [cursor2Pos, setCursor2Pos] = useState<{x:number, y:number}>({x:0, y:0})
  ...
  useEffect(() => {
    gsap.set(cursor.current, {
      x: lerp(posX.current, cursor2Pos?.x as number, 0.1) - shape.current?.offsetWidth! / 2,
      y: lerp(posY.current, cursor2Pos?.y as number, 0.1) - shape.current?.offsetHeight! / 2
    })
  }, [cursor2Pos])
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full z-cursor pointer-events-none">
      <div ref={cursor} className="absolute w-[60px] h-[60px] flex justify-center items-center backdrop-blur-[3px] rounded-full border-white border-[3px] overflow-hidden">
        ...
        <div ref={iconW} className="cursor__icons absolute top-0 left-0 w-full h-full" >
          <div ref={icons} className="absolute top-[25%] left-[25%] w-[50%] h-[50%] overflow-hidden">
            <div ref={muteIcon} className="absolute top-0 lef-0 w-full h-full"><Mute classname="absolute top-0 left-0 w-full h-full"/></div>
            <div ref={unmuteIcon} className="absolute top-0 lef-0 w-full h-full"><Unmute classname="absolute top-0 left-0 w-full h-full"/></div>
          </div>
        </div>

      </div>
      <div ref={cursor2} className="hidden absolute w-[60px] h-[60px] rounded-full border-error border-[3px] overflow-hidden"></div>
    </div>
  )
}
