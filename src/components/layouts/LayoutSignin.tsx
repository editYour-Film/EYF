...
const LayoutSignin: React.FC<layoutProps> = ({ children }) => {
  ...
  const handleMousMove = (e:MouseEvent) => {
    if (bgGradient.current && content.current) {
      ...
      bgInstantPos.x = xPosNormal * xOffsetBg + (window.innerWidth / 2 - bgGradient.current?.offsetWidth / 2)
      bgInstantPos.y = yPosNormal * yOffsetBg + (window.innerHeight / 2 - bgGradient.current?.offsetHeight / 2)

      const lateAnim = gsap.to(bgAbstractPos, {
        x: (xPosNormal * xOffsetBg) + (window.innerWidth / 2 - bgGradient.current?.offsetWidth / 2),
        y: (yPosNormal * yOffsetBg) + (window.innerHeight / 2 - bgGradient.current?.offsetHeight / 2),
        duration: lateDuration,
        ease: 'power2.out',

        onUpdate: () => {
          const lerpx = lerp(bgInstantPos.x, bgAbstractPos.x, 1 - lateAnim.progress())
          const lerpy = lerp(bgInstantPos.y, bgAbstractPos.y, 1 - lateAnim.progress())
  ...
  return (
    <>
      <div className={"bg-black relative min-h-screen overflow-hidden"}>
        <div ref={bgGradient} className="bg-signin absolute w-[40vw] min-w-[500px] h-[40vw] min-h-[500px] -z-1 pointer-events-none sm:-left-0 sm:left-1/8"></div>
        <main className={`flex min-h-[calc(100vh)] w-full justify-center items-center perspective`}>
          <div ref={content} 
